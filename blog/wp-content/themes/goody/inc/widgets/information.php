...
add_action('widgets_init', 'goody_information');

function goody_information()
{
...
class information_widget extends WP_Widget
{
    ...
    {
        /* Widget settings. */
        $widget_ops = array('classname' => 'information_widget', 'description' => esc_html__('Easy add image for sidebar.', 'goody'));

        /* Widget control settings. */
        $control_ops = array('width' => 250, 'height' => 350, 'id_base' => 'information_widget');

        /* Create the widget. */
        parent::__construct('information_widget', esc_html__('+NA: Image', 'goody'), $widget_ops, $control_ops);
    }
        ...
        <div class="goody-image-content">
            <?php if ($image) {
                ...
                <a class="clearfix" href="<?php echo esc_url($link); ?>">
                    <img class="goody-image" src="<?php echo esc_url($image)?>" alt="img" />
                </a>
            <?php } ?>
        </div>
        <?php
    ...
    function form($instance)
    {
        ...
        <p id="<?php echo esc_attr($this->get_field_id('image').'-wrapp'); ?>">
            <label for="<?php echo esc_attr($this->get_field_id('image')); ?>"><?php esc_html_e('Image:', 'goody'); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('link')); ?>"><?php echo esc_html_e('Url:', 'goody'); ?></label>
