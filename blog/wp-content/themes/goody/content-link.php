...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<header class="entry-header">
		<?php
			if ( is_single() ) :
				the_title( sprintf( '<h1 class="entry-title"><a href="%s">', esc_url( goody_get_link_url() ) ), '</a></h1>' );
			else :
				the_title( sprintf( '<h2 class="entry-title"><a href="%s">', esc_url( goody_get_link_url() ) ), '</a></h2>' );
			endif;
		?>
	</header>
	<!-- .entry-header -->
	...
	<div class="entry-content">
		<?php
			...
			the_content( sprintf(
				esc_html__ 'Continue reading %s', 'goody' ),
